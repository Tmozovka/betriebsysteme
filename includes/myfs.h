...
public:
	...
	int fuseRead(const char *path, char *buf, size_t size, off_t offset,
			struct fuse_file_info *fileInfo); //implement ??
...
#else
	int fuseSetxattr(const char *path, const char *name, const char *value,
			size_t size, int flags);
	int fuseGetxattr(const char *path, const char *name, char *value,
			size_t size);
#endif
	...
	int fuseReaddir(const char *path, void *buf, fuse_fill_dir_t filler,
			off_t offset, struct fuse_file_info *fileInfo); //implement
	int fuseReleasedir(const char *path, struct fuse_file_info *fileInfo); //implement
	int fuseFsyncdir(const char *path, int datasync,
	...
	int addFile(const char * name, mode_t mode, time_t mtime, off_t size,
			char * text);
